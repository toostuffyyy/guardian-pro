...
public class GroupVisitViewModel : ViewModelBase
{
    ...
    private VisitorDTO _selectedVisitorDto;
    ...
    // Свойство SelectedVisitorDto.
    public VisitorDTO SelectedVisitorDto
    {
        get => _selectedVisitorDto;
        set => this.RaiseAndSetIfChanged(ref _selectedVisitorDto, value);
    }
    // Конструктор.
    public GroupVisitViewModel()
    {
        ...
        CreateOrderDto = new CreateOrderDTO() { TypeOrderId = 2 };
        ...
        if (responce != null && responce.StatusCode == HttpStatusCode.OK)
        {
            Dispatcher.UIThread.InvokeAsync(() =>
            {
                Locator.Current.GetService<INotificationManager>()
                    .Show(new Notification("Заявка", "Заявка оформлена", NotificationType.Success));
            });
        }
        else
        {
            ...
            {
                Locator.Current.GetService<INotificationManager>()
                    .Show(new Notification("Ошибка", "Заявка не оформлена.", NotificationType.Error));
            });
        }
    }
    // Метод для добавления постетителя в DataGrid.
    public void AddVisitor()
    {
        CreateOrderDto.Visitors.Add(SelectedVisitorDto);
        SelectedVisitorDto = new VisitorDTO();
    }
